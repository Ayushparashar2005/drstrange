    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if 0.5 < ratio < 1.3:
                ...
                if diameter > 0:
                    filter_state.deg = (filter_state.deg + config["overlay"]["rotation_degree_increment"]) % 360
                    M1 = cv.getRotationMatrix2D((outer_circle.shape[1] // 2, outer_circle.shape[0] // 2), filter_state.deg, 1.0)
                    M2 = cv.getRotationMatrix2D((inner_circle.shape[1] // 2, inner_circle.shape[0] // 2), -filter_state.deg, 1.0)
